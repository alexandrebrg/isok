...
impl Broker for BrokerGrpcService {
    ...
    async fn batch_send(&self, mut request: tonic::Request<CheckBatchRequest>) -> Result<tonic::Response<CheckBatchResponse>, tonic::Status> {
        ...
        tracing::debug!("Received a new batch of events, length: {}", request.get_ref().events.len());
        ...
        for event in request.get_ref().events.iter() {
            self.message_broker.process_message(event).await
                .map_err(|e| tonic::Status::internal(e.to_string()))?;
        }
        Ok(tonic::Response::new(CheckBatchResponse {
            error: None,
        }))
    }
...
impl BrokerGrpcService {
    ...
    pub async fn run_on(self, addr: std::net::SocketAddr) -> Result<(), ApiError> {
        ...
        Server::builder()
            .add_service(server)
            .serve(addr)
            .await.map_err(ApiError::ServerFailure)
    }
}